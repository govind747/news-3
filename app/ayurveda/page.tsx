...
const SUBCATEGORIES_CONFIG = [
  ...
  { key: 'Herbal Remedies', en: 'Herbal Remedies', hi: 'जड़ी-बूटियाँ' },
  { key: 'Home Remedies', en: 'Home Remedies', hi: 'घरेलू उपचार' },
  { key: 'Doshas', en: 'Doshas', hi: 'त्रिदोष (वात/पित्त/कफ)' },
  { key: 'Diet & Nutrition', en: 'Diet & Nutrition', hi: 'आहार एवं पोषण' },
  { key: 'Lifestyle & Yoga', en: 'Lifestyle & Yoga', hi: 'जीवनशैली एवं योग' },
  { key: 'Wellness', en: 'Wellness', hi: 'स्वास्थ्य' },
];
...
export default function AyurvedaPage() {
  ...
  const t = {
    ...
    breadcrumbAyurveda: isHi ? 'आयुर्वेद एवं स्वास्थ्य' : 'Ayurveda & Health',
    badge: isHi ? '🌿 आयुर्वेदिक उपचार एवं स्वास्थ्य' : '🌿 Ayurvedic Health & Wellness',
    pageTitle: isHi ? 'आयुर्वेदिक प्राकृतिक स्वास्थ्य कोश' : 'Ayurvedic Health & Natural Healing',
    pageDesc: isHi
      ? 'पारंपरिक भारतीय चिकित्सा, त्रिदोष संतुलन, जड़ी-बूटियों और प्राकृतिक उपचारों पर आधारित गाइड।'
      : 'Explore comprehensive guides on traditional remedies, dosha balance, herbal cures, and holistic wellness.',
    searchPlaceholder: isHi ? 'आयुर्वेदिक स्वास्थ्य लेख खोजें...' : 'Search ayurvedic remedies & tips...',
    ...
    doshaLabel: isHi ? 'दोष' : 'Dosha',
    herbLabel: isHi ? 'मुख्य बूटी' : 'Herb',
    read: isHi ? 'पढ़ें →' : 'Read →',
    noResults: isHi ? 'आपकी खोज के अनुसार कोई स्वास्थ्य लेख नहीं मिला।' : 'No articles found matching your criteria.',
    loadingText: isHi ? 'आयुर्वेदिक स्वास्थ्य लेख लोड हो रहे हैं...' : 'Loading ayurvedic health guides...',
    minRead: isHi ? 'मिनट' : 'min',
    promotedTitle: isHi ? 'विशेष स्वास्थ्य लेख' : 'Featured Health Guides',
  };
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      // Fetch Articles categorized under 'ayurveda'
      let query = supabase
        .from('articles')
        .select('*')
  ...
  // Slicing post segments for custom sections
    ...
    <>
      ...
      <div className="bg-[#F8F8F8] border-b border-[#E8E8E8] py-3">
        <div className="max-w-7xl mx-auto px-4 text-xs font-body text-[#999]">
          ...
          <span className="text-[#111]">{t.breadcrumbAyurveda}</span>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-4">
        <AdBanner slot="ayurveda-top" size="leaderboard" />
      </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            {loading ? (
              ...
              <>
                ...
                {heroPost && (
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-6 mb-10 pb-8 border-b border-[#E8E8E8]">
                    ...
                    <div className="md:col-span-7 group">
                      <Link href={`/ayurveda/${heroPost.slug}${isHi ? '?lang=hi' : ''}`}>
                        <div className="overflow-hidden mb-3 relative aspect-[16/10]">
                          <img
                            src={heroPost.image_url || 'https://images.pexels.com/photos/3785079/pexels-photo-3785079.jpeg'}
                            alt={isHi && heroPost.title_hi ? heroPost.title_hi : heroPost.title}
                            className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                          />
                          {(heroPost.dosha_type || heroPost.lucky_number) && (
                            <span className="absolute top-3 left-3 bg-brand text-white text-xs font-bold px-2.5 py-1">
                              {t.doshaLabel}: {heroPost.dosha_type || heroPost.lucky_number}
                            </span>
                          )}
                        </div>
                    ...
                    <div className="md:col-span-5 flex flex-col gap-4">
                      {rightListPosts.map((post) => (
                        <Link
                          key={post.id}
                          href={`/ayurveda/${post.slug}${isHi ? '?lang=hi' : ''}`}
                          className="group flex gap-3 pb-3 border-b border-[#F0F0F0] last:border-0"
                        >
                          <img
                            src={post.image_url || 'https://images.pexels.com/photos/3785079/pexels-photo-3785079.jpeg'}
                  ...
                  <div className="mb-10 pb-8 border-b border-[#E8E8E8]">
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                      {fourSquarePosts1.map((post) => (
                        <Link key={post.id} href={`/ayurveda/${post.slug}${isHi ? '?lang=hi' : ''}`} className="group block">
                  ...
                  <div className="mb-10 pb-8 border-b border-[#E8E8E8]">
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                      {sixGridPosts.map((post) => (
                        <Link key={post.id} href={`/ayurveda/${post.slug}${isHi ? '?lang=hi' : ''}`} className="group block border border-[#E8E8E8] bg-white p-3">
                ...
                {/* 6. GOOGLE AD BANNER */}
                <div className="my-8">
                  <AdBanner slot="ayurveda-infeed" size="leaderboard" />
                </div>
                  ...
                  <div className="bg-[#FFFDF5] border border-[#F5E6B3] p-6 mb-10">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      {promotedPosts.map((post) => (
                        <Link key={post.id} href={`/ayurveda/${post.slug}${isHi ? '?lang=hi' : ''}`} className="group flex gap-4">
                          <img
                            src={post.image_url || 'https://images.pexels.com/photos/3785079/pexels-photo-3785079.jpeg'}
                            alt={isHi && post.title_hi ? post.title_hi : post.title}
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 pb-8">
        <AdBanner slot="ayurveda-bottom" size="leaderboard" />
      </div>
    </>
  );
}