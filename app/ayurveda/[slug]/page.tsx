...
export async function generateStaticParams() {
  const sb = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!);
  const { data } = await sb.from('articles').select('slug').eq('category', 'ayurveda');
...
export async function generateMetadata({ params }: Props) {
  ...
  return {
    title: a?.meta_title || a?.title || 'Ayurveda Article – VedaWell',
    description: a?.meta_description || a?.excerpt || 'Ayurvedic herbs and healing on VedaWell.',
...
export default function AyurvedaArticlePage({ params }: Props) {
  return <ArticleDetailLayout slug={params.slug} categorySlug="ayurveda" categoryLabel="Ayurveda" />;
}
