...
export async function generateStaticParams() {
  const sb = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!);
  const { data } = await sb.from('articles').select('slug').eq('category', 'nutrition');
...
export async function generateMetadata({ params }: Props) {
  ...
  return {
    title: a?.meta_title || a?.title || 'Nutrition Article – VedaWell',
    description: a?.meta_description || a?.excerpt || 'Nutrition and diet guides on VedaWell.',
...
export default function NutritionArticlePage({ params }: Props) {
  return <ArticleDetailLayout slug={params.slug} categorySlug="nutrition" categoryLabel="Nutrition & Diet" />;
}
