...
const SUBCATEGORIES_CONFIG = [
  ...
  { key: 'Diet Plans', en: 'Diet Plans', hi: 'आहार योजनाएं' },
  { key: 'Superfoods', en: 'Superfoods', hi: 'सुपरफूड्स' },
  { key: 'Vitamins & Minerals', en: 'Vitamins & Minerals', hi: 'विटामिन और खनिज' },
  { key: 'Weight Loss', en: 'Weight Loss', hi: 'वजन घटाना' },
  { key: 'Gut Health', en: 'Gut Health', hi: 'पाचन स्वास्थ्य' },
  { key: 'Ayurvedic Diet', en: 'Ayurvedic Diet', hi: 'आयुर्वेदिक आहार' },
];
...
export default function NutritionPage() {
  ...
  const t = {
    ...
    breadcrumbNutrition: isHi ? 'पोषण एवं आहार' : 'Nutrition & Diet',
    badge: isHi ? '🥗 पोषण एवं स्वस्थ आहार' : '🥗 Nutrition & Diet Guide',
    pageTitle: isHi ? 'पोषण एवं डाइट ज्ञानकोश' : 'Nutrition, Diet & Wellness Encyclopedia',
    pageDesc: isHi
      ? 'संतुलित आहार, सुपरफूड्स, विटामिन, वजन घटाने के टिप्स और विशेषज्ञ पोषण सलाह प्राप्त करें।'
      : 'Explore evidence-based nutrition guides, superfoods, meal plans, vitamins, and holistic dietary advice.',
    searchPlaceholder: isHi ? 'पोषण विषय, सुपरफूड्स या डाइट टिप्स खोजें...' : 'Search nutrition topics, superfoods & diet plans...',
    ...
    of: isHi ? 'में से' : 'of',
    nutritionCount: isHi ? 'लेख' : 'articles',
    lucky: isHi ? 'डाइट कोड' : 'Diet Tip',
    read: isHi ? 'पढ़ें →' : 'Read →',
    noResults: isHi ? 'आपकी खोज के अनुसार कोई पोषण लेख नहीं मिला।' : 'No nutrition articles found matching your criteria.',
    loadingText: isHi ? 'पोषण गाइड लोड हो रही है...' : 'Loading nutrition & dietary articles...',
    minRead: isHi ? 'मिनट' : 'min',
    promotedTitle: isHi ? 'प्रमुख पोषण गाइड' : 'Featured Nutrition Articles',
  };

  // Fetch Nutrition Category Articles and DB Ads
  useEffect(() => {
    async function fetchData() {
      setLoading(true);
      
      // Fetch Nutrition Category Articles
      let query = supabase
        .from('articles')
        .select('*')
  ...
  const defaultNutritionImage = 'https://images.unsplash.com/photo-1490645935967-10de6ba17061?auto=format&fit=crop&w=1200&q=80';
    ...
    <>
      ...
      <div className="bg-[#F8F8F8] border-b border-[#E8E8E8] py-3">
        <div className="max-w-7xl mx-auto px-4 text-xs font-body text-[#999]">
          ...
          <span className="text-[#111]">{t.breadcrumbNutrition}</span>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-4">
        <AdBanner slot="nutrition-top" size="leaderboard" />
      </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            {loading ? (
              ...
              <>
                ...
                {heroPost && (
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-6 mb-10 pb-8 border-b border-[#E8E8E8]">
                    ...
                    <div className="md:col-span-7 group">
                      <Link href={`/nutrition/${heroPost.slug}${isHi ? '?lang=hi' : ''}`}>
                        <div className="overflow-hidden mb-3 relative aspect-[16/10]">
                          <img
                            src={heroPost.image_url || defaultNutritionImage}
                    ...
                    <div className="md:col-span-5 flex flex-col gap-4">
                      {rightListPosts.map((post) => (
                        <Link
                          key={post.id}
                          href={`/nutrition/${post.slug}${isHi ? '?lang=hi' : ''}`}
                          className="group flex gap-3 pb-3 border-b border-[#F0F0F0] last:border-0"
                        >
                          <img
                            src={post.image_url || defaultNutritionImage}
                  ...
                  <div className="mb-10 pb-8 border-b border-[#E8E8E8]">
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                      {fourSquarePosts1.map((post) => (
                        <Link key={post.id} href={`/nutrition/${post.slug}${isHi ? '?lang=hi' : ''}`} className="group block">
                  ...
                  <div className="mb-10 pb-8 border-b border-[#E8E8E8]">
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                      {sixGridPosts.map((post) => (
                        <Link key={post.id} href={`/nutrition/${post.slug}${isHi ? '?lang=hi' : ''}`} className="group block border border-[#E8E8E8] bg-white p-3">
                ...
                {/* 6. GOOGLE AD BANNER */}
                <div className="my-8">
                  <AdBanner slot="nutrition-infeed" size="leaderboard" />
                </div>
                  ...
                  <div className="bg-[#FFFDF5] border border-[#F5E6B3] p-6 mb-10">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      {promotedPosts.map((post) => (
                        <Link key={post.id} href={`/nutrition/${post.slug}${isHi ? '?lang=hi' : ''}`} className="group flex gap-4">
                          <img
                            src={post.image_url || defaultNutritionImage}
                            alt={isHi && post.title_hi ? post.title_hi : post.title}
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 pb-8">
        <AdBanner slot="nutrition-bottom" size="leaderboard" />
      </div>
    </>
  );
}