...
export async function generateStaticParams() {
  const sb = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!);
  const { data } = await sb.from('articles').select('slug').eq('category', 'spirituality');
...
export async function generateMetadata({ params }: Props) {
  ...
  return {
    title: a?.meta_title || a?.title || 'Spirituality Article – VedaWell',
    description: a?.meta_description || a?.excerpt || 'Vedic spirituality on VedaWell.',
...
export default function SpiritualityArticlePage({ params }: Props) {
  return <ArticleDetailLayout slug={params.slug} categorySlug="spirituality" categoryLabel="Spirituality" />;
}
