...
const SUBCATEGORIES_CONFIG = [
  ...
  { key: 'Meditation', en: 'Meditation', hi: 'ध्यान और साधना' },
  { key: 'Philosophy', en: 'Philosophy', hi: 'आध्यात्मिक दर्शन' },
  { key: 'Energy Healing', en: 'Energy Healing', hi: 'ऊर्जा चिकित्सा' },
  { key: 'Mindset & Abundance', en: 'Mindset & Abundance', hi: 'मानसिकता और समृद्धि' },
  { key: 'Sacred Wisdom', en: 'Sacred Wisdom', hi: 'प्राचीन रहस्य' },
];
...
export default function SpiritualityPage() {
  ...
  const t = {
    ...
    breadcrumbSpirituality: isHi ? 'आध्यात्मिकता और ध्यान' : 'Spirituality & Mind',
    badge: isHi ? '🕉️ आध्यात्मिकता और आत्म-ज्ञान' : '🕉️ Spiritual Wisdom & Mindfulness',
    pageTitle: isHi ? 'आध्यात्मिक एवं चेतना ज्ञानकोश' : 'Spirituality & Consciousness Encyclopedia',
    pageDesc: isHi
      ? 'ध्यान, चक्र संतुलन, कर्म का सिद्धांत, मन की शांति और प्राचीन आध्यात्मिक रहस्यों को गहराई से समझें।'
      : 'Explore mindfulness, chakra balancing, universal laws, meditation techniques, and ancient spiritual philosophy.',
    searchPlaceholder: isHi ? 'आध्यात्मिक विषय, ध्यान, चक्र या मंत्र खोजें...' : 'Search spiritual topics, meditation & chakras...',
    ...
    of: isHi ? 'में से' : 'of',
    spiritualityCount: isHi ? 'लेख' : 'articles',
    lucky: isHi ? 'मंत्र / सूत्र' : 'Wisdom Code',
    read: isHi ? 'पढ़ें →' : 'Read →',
    noResults: isHi ? 'आपकी खोज के अनुसार कोई आध्यात्मिक लेख नहीं मिला।' : 'No spiritual articles found matching your criteria.',
    loadingText: isHi ? 'आध्यात्मिक मार्गदर्शिका लोड हो रही है...' : 'Loading spiritual & mindfulness guides...',
    minRead: isHi ? 'मिनट' : 'min',
    promotedTitle: isHi ? 'प्रमुख आध्यात्मिक लेख' : 'Featured Spiritual Articles',
  };

  // Fetch Spirituality Category Articles and DB Ads
  useEffect(() => {
    async function fetchData() {
      setLoading(true);
      
      // Fetch Spirituality Category Articles
      let query = supabase
        .from('articles')
        .select('*')
  ...
  const defaultSpiritualityImage = 'https://images.unsplash.com/photo-1506126613408-eca07ce68773?auto=format&fit=crop&w=1200&q=80';
    ...
    <>
      ...
      <div className="bg-[#F8F8F8] border-b border-[#E8E8E8] py-3">
        <div className="max-w-7xl mx-auto px-4 text-xs font-body text-[#999]">
          ...
          <span className="text-[#111]">{t.breadcrumbSpirituality}</span>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-4">
        <AdBanner slot="spirituality-top" size="leaderboard" />
      </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            {loading ? (
              ...
              <>
                ...
                {heroPost && (
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-6 mb-10 pb-8 border-b border-[#E8E8E8]">
                    ...
                    <div className="md:col-span-7 group">
                      <Link href={`/spirituality/${heroPost.slug}${isHi ? '?lang=hi' : ''}`}>
                        <div className="overflow-hidden mb-3 relative aspect-[16/10]">
                          <img
                            src={heroPost.image_url || defaultSpiritualityImage}
                    ...
                    <div className="md:col-span-5 flex flex-col gap-4">
                      {rightListPosts.map((post) => (
                        <Link
                          key={post.id}
                          href={`/spirituality/${post.slug}${isHi ? '?lang=hi' : ''}`}
                          className="group flex gap-3 pb-3 border-b border-[#F0F0F0] last:border-0"
                        >
                          <img
                            src={post.image_url || defaultSpiritualityImage}
                  ...
                  <div className="mb-10 pb-8 border-b border-[#E8E8E8]">
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                      {fourSquarePosts1.map((post) => (
                        <Link key={post.id} href={`/spirituality/${post.slug}${isHi ? '?lang=hi' : ''}`} className="group block">
                  ...
                  <div className="mb-10 pb-8 border-b border-[#E8E8E8]">
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                      {sixGridPosts.map((post) => (
                        <Link key={post.id} href={`/spirituality/${post.slug}${isHi ? '?lang=hi' : ''}`} className="group block border border-[#E8E8E8] bg-white p-3">
                ...
                {/* 6. GOOGLE AD BANNER */}
                <div className="my-8">
                  <AdBanner slot="spirituality-infeed" size="leaderboard" />
                </div>
                  ...
                  <div className="bg-[#FFFDF5] border border-[#F5E6B3] p-6 mb-10">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      {promotedPosts.map((post) => (
                        <Link key={post.id} href={`/spirituality/${post.slug}${isHi ? '?lang=hi' : ''}`} className="group flex gap-4">
                          <img
                            src={post.image_url || defaultSpiritualityImage}
                            alt={isHi && post.title_hi ? post.title_hi : post.title}
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 pb-8">
        <AdBanner slot="spirituality-bottom" size="leaderboard" />
      </div>
    </>
  );
}