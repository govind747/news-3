...
const SUBCATEGORIES_CONFIG = [
  ...
  { key: 'Animals', en: 'Animals', hi: 'जानवर' },
  { key: 'Nature', en: 'Nature', hi: 'प्रकृति' },
  { key: 'Elements', en: 'Elements', hi: 'तत्व' },
  { key: 'People', en: 'People', hi: 'लोग' },
  { key: 'Objects', en: 'Objects', hi: 'वस्तुएं' },
  { key: 'Situations', en: 'Situations', hi: 'परिस्थितियां' },
];
...
export default function DreamsPage() {
  ...
  const t = {
    ...
    breadcrumbDreams: isHi ? 'स्वप्न फल' : 'Dream Meanings',
    badge: isHi ? '🌙 स्वप्न व्याख्या' : '🌙 Dream Interpretation',
    pageTitle: isHi ? 'स्वप्न फल एवं व्याख्या कोश' : 'Dream Meanings Encyclopedia',
    pageDesc: isHi
      ? 'हिंदू, पौराणिक और आध्यात्मिक परंपराओं पर आधारित स्वप्न फलों का अन्वेषण करें।'
      : 'Explore dream interpretations rooted in traditional, Vedic, and spiritual insights.',
    searchPlaceholder: isHi ? 'सपने का अर्थ खोजें...' : 'Search dream meanings...',
    show: isHi ? 'दिखाएं:' : 'Show:',
    postsLabel: isHi ? 'पोस्ट' : 'posts',
    newest: isHi ? 'नवीनतम' : 'Newest First',
    popular: isHi ? 'लोकप्रिय' : 'Most Popular',
    az: isHi ? 'अ → ज' : 'A → Z',
    showing: isHi ? 'दिखा रहा है' : 'Showing',
    of: isHi ? 'में से' : 'of',
    dreamsCount: isHi ? 'सपने' : 'dreams',
    lucky: isHi ? 'शुभ' : 'Lucky',
    read: isHi ? 'पढ़ें →' : 'Read →',
    noResults: isHi ? 'आपकी खोज के अनुसार कोई सपना नहीं मिला।' : 'No dreams found matching your criteria.',
    ...
    <>
      ...
      <div className="bg-[#F8F8F8] border-b border-[#E8E8E8] py-3">
        <div className="max-w-7xl mx-auto px-4 text-xs font-body text-[#999]">
          ...
          <span className="text-[#111]">{t.breadcrumbDreams}</span>
        </div>
      </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            {loading ? (
              ...
              <>
                ...
                {heroPost && (
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-6 mb-10 pb-8 border-b border-[#E8E8E8]">
                    ...
                    <div className="md:col-span-7 group">
                      <Link href={`/dreams/${heroPost.slug}${isHi ? '?lang=hi' : ''}`}>
                    ...
                    <div className="md:col-span-5 flex flex-col gap-4">
                      {rightListPosts.map((post) => (
                        <Link
                          key={post.id}
                          href={`/dreams/${post.slug}${isHi ? '?lang=hi' : ''}`}
                  ...
                  <div className="mb-10 pb-8 border-b border-[#E8E8E8]">
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                      {sixGridPosts.map((post) => (
                        <Link key={post.id} href={`/dreams/${post.slug}${isHi ? '?lang=hi' : ''}`} className="group block border border-[#E8E8E8] bg-white p-3">
                ...
                {/* 6. GOOGLE AD BANNER */}
                <div className="my-8">
                  <AdBanner slot="dreams-infeed" size="leaderboard" />
                </div>
                  ...
                  <div className="bg-[#FFFDF5] border border-[#F5E6B3] p-6 mb-10">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      {promotedPosts.map((post) => (
                        <Link key={post.id} href={`/dreams/${post.slug}${isHi ? '?lang=hi' : ''}`} className="group flex gap-4">
                          <img
                            src={post.image_url || 'https://images.pexels.com/photos/3785079/pexels-photo-3785079.jpeg'}
                            alt={isHi && post.title_hi ? post.title_hi : post.title}
