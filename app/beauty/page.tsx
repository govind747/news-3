...
const SUBCATEGORIES_CONFIG = [
  ...
  { key: 'Skin Care', en: 'Skin Care', hi: 'त्वचा की देखभाल' },
  { key: 'Hair Care', en: 'Hair Care', hi: 'बालों की देखभाल' },
  { key: 'DIY Home Remedies', en: 'DIY Remedies', hi: 'घरेलू नुस्खे' },
  { key: 'Anti-Aging', en: 'Anti-Aging', hi: 'एंटी-एजिंग' },
  { key: 'Natural Cosmetics', en: 'Natural Cosmetics', hi: 'प्राकृतिक सौंदर्य' },
  { key: 'Body Care', en: 'Body Care', hi: 'शरीर की देखभाल' },
];
...
export default function BeautyPageClient() {
  ...
  const t = {
    ...
    breadcrumbBeauty: isHi ? 'सौंदर्य एवं त्वचा देखभाल' : 'Beauty & Skincare',
    badge: isHi ? '✨ प्राकृतिक सौंदर्य एवं स्किनकेयर' : '✨ Natural Beauty & Skincare Guide',
    pageTitle: isHi ? 'प्राकृतिक सौंदर्य एवं स्किनकेयर कोश' : 'Natural Beauty & Skincare Secrets',
    pageDesc: isHi
      ? 'चमकती त्वचा, लंबे और घने बाल, प्राकृतिक फेस पैक और आयुर्वेदिक ब्यूटी टिप्स पर आधारित प्रामाणिक गाइड।'
      : 'Explore holistic beauty secrets, DIY natural face masks, hair growth remedies, and expert dermatologist-backed routines.',
    searchPlaceholder: isHi ? 'सौंदर्य और त्वचा संबंधी लेख खोजें...' : 'Search beauty, hair & skincare tips...',
    show: isHi ? 'दिखाएं:' : 'Show:',
    postsLabel: isHi ? 'लेख' : 'articles',
    newest: isHi ? 'नवीनतम' : 'Newest First',
    popular: isHi ? 'लोकप्रिय' : 'Most Popular',
    az: isHi ? 'अ → ज' : 'A → Z',
    showing: isHi ? 'दिखा रहा है' : 'Showing',
    of: isHi ? 'में से' : 'of',
    articlesCount: isHi ? 'लेख' : 'articles',
    skinTypeLabel: isHi ? 'त्वचा प्रकार' : 'Skin Type',
    herbLabel: isHi ? 'मुख्य घटक' : 'Key Ingredient',
    read: isHi ? 'पढ़ें →' : 'Read →',
    noResults: isHi ? 'आपकी खोज के अनुसार कोई सौंदर्य लेख नहीं मिला।' : 'No beauty articles found matching your criteria.',
    loadingText: isHi ? 'सौंदर्य एवं स्किनकेयर लेख लोड हो रहे हैं...' : 'Loading beauty & skincare guides...',
    minRead: isHi ? 'मिनट' : 'min',
    promotedTitle: isHi ? 'विशेष ब्यूटी गाइड्स' : 'Featured Beauty Guides',
  };
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      // Fetch Articles categorized under 'beauty'
      let query = supabase
        .from('articles')
        .select('*')
        .eq('category', 'beauty')
  ...
  // Slicing post segments for custom grid sections
  ...
  const fallbackImg = 'https://images.pexels.com/photos/3785079/pexels-photo-3785079.jpeg?auto=compress&cs=tinysrgb&w=800';
    ...
    <>
      ...
      <div className="bg-[#F8F8F8] border-b border-[#E8E8E8] py-3">
        <div className="max-w-7xl mx-auto px-4 text-xs font-body text-[#999]">
          ...
          <span className="text-[#111]">{t.breadcrumbBeauty}</span>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-4">
        <AdBanner slot="beauty-top" size="leaderboard" />
      </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            {loading ? (
              ...
              <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-6 mb-10 pb-8 border-b border-[#E8E8E8]">
                    ...
                    <div className="md:col-span-5 flex flex-col gap-4">
                      {rightListPosts.map((post) => (
                        <Link
                          key={post.id}
                          href={`/beauty/${post.slug}${isHi ? '?lang=hi' : ''}`}
                ...
                {/* 2. DATABASE ADS BANNER (Auto-Changing Ads from Supabase) */}
                {activeAd && (
                  ...
                  <div className="mb-10 pb-8 border-b border-[#E8E8E8]">
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                      {sixGridPosts.map((post) => (
                        <Link key={post.id} href={`/beauty/${post.slug}${isHi ? '?lang=hi' : ''}`} className="group block border border-[#E8E8E8] bg-white p-3">
                ...
                {/* 6. GOOGLE AD BANNER */}
                <div className="my-8">
                  <AdBanner slot="beauty-infeed" size="leaderboard" />
                </div>

                {/* 7. SECTION: Promoted Beauty Articles */}
                  ...
                  <div className="bg-[#FFFDF5] border border-[#F5E6B3] p-6 mb-10">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      {promotedPosts.map((post) => (
                        <Link key={post.id} href={`/beauty/${post.slug}${isHi ? '?lang=hi' : ''}`} className="group flex gap-4">
                          <img
                            src={post.image_url || fallbackImg}
                            alt={isHi && post.title_hi ? post.title_hi : post.title}
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 pb-8">
        <AdBanner slot="beauty-bottom" size="leaderboard" />
      </div>
    </>
  );
}