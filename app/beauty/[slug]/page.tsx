...
export async function generateStaticParams() {
  const sb = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!);
  const { data } = await sb.from('articles').select('slug').eq('category', 'beauty');
...
export async function generateMetadata({ params }: Props) {
  ...
  return {
    title: a?.meta_title || a?.title || 'Beauty Article – VedaWell',
    description: a?.meta_description || a?.excerpt || 'Natural beauty tips on VedaWell.',
...
export default function BeautyArticlePage({ params }: Props) {
  return <ArticleDetailLayout slug={params.slug} categorySlug="beauty" categoryLabel="Beauty & Skincare" />;
}
