...
export async function generateStaticParams() {
  const sb = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!);
  const { data } = await sb.from('articles').select('slug').eq('category', 'yoga');
...
export async function generateMetadata({ params }: Props) {
  ...
  return {
    title: a?.meta_title || a?.title || 'Yoga Article – VedaWell',
    description: a?.meta_description || a?.excerpt || 'Yoga and meditation guides on VedaWell.',
...
export default function YogaArticlePage({ params }: Props) {
  return <ArticleDetailLayout slug={params.slug} categorySlug="yoga" categoryLabel="Yoga & Meditation" />;
}
