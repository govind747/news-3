...
import { Clock, Tag, Share2, Heart, Bookmark, Eye, Loader2, Facebook, Twitter } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="flex flex-col lg:flex-row gap-10">
          ...
          <article className="flex-1 min-w-0">
            ...
            <AdBanner slot="banner-2" category={categorySlug} className="mb-6" />

            {/* Article Content */}
            <div className="article-body mb-8" dangerouslySetInnerHTML={{ __html: displayContent }} />

            {/* Bottom Banner Ad */}
            <AdBanner slot="banner-3" category={categorySlug} className="my-6" />
            ...
            <div>
              <div className="divider-title mb-4">
                <h2 className="text-sm font-bold uppercase tracking-widest">{lang === 'hi' ? 'टिप्पणी करें' : 'Leave a Comment'}</h2>
              </div>
              <div className="bg-[#F8F8F8] p-6 border border-[#E8E8E8]">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                  <input type="text" placeholder={lang === 'hi' ? 'आपका नाम' : 'Your Name'} className="px-4 py-3 border border-[#E8E8E8] text-sm bg-white focus:outline-none focus:border-brand font-body" />
                  <input type="email" placeholder={lang === 'hi' ? 'आपका ईमेल' : 'Your Email'} className="px-4 py-3 border border-[#E8E8E8] text-sm bg-white focus:outline-none focus:border-brand font-body" />
                </div>
                <textarea rows={4} placeholder={lang === 'hi' ? 'अपने विचार साझा करें...' : 'Share your thoughts...'} className="w-full px-4 py-3 border border-[#E8E8E8] text-sm bg-white focus:outline-none focus:border-brand mb-4 font-body resize-none" />
                <button className="btn-primary">{lang === 'hi' ? 'टिप्पणी पोस्ट करें' : 'Post Comment'}</button>
              </div>
            </div>
          </article>
          ...
          <div className="lg:w-80 flex-shrink-0">
            <div className="sticky-sidebar space-y-6">
              <Sidebar />
              <AdBanner slot="side-image-1" category={categorySlug} />
              <AdBanner slot="side-image-2" category={categorySlug} />
              <AdBanner slot="side-image-3" category={categorySlug} />
            </div>
          </div>
        </div>
      </div>
